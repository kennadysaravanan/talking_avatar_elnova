...
from utils.detectors import S3FD
...
def crop_video(opt, track, cropfile):
    ...
    command = ("ffmpeg -v quiet -y -i %s -ss %.3f -to %.3f %s" % (
    ...
    command = ("ffmpeg -v quiet -y -i %st.avi -i %s -c:v copy -c:a copy %s.avi" % (cropfile, audiotmp, cropfile))
    output = subprocess.call(command, shell=True, stdout=None)
...
def run_syncnet(videofile, data_dir):
    # ========== ========== ========== ==========
    # # PARSE ARGS
    # ========== ========== ========== ==========
    global syncnet_inst
    ...
    opt = attrdict.AttrDict({
        ...
        'min_track': 25, 'frame_rate': 25,
        'num_failed_det': 25, 'min_face_size': 48,
    ...
    out_video = os.path.join(opt.avi_dir, opt.reference, 'video.avi')
    command = ("ffmpeg -y -i %s -ss 00:00:00 -to 00:00:0%s -qscale:v 2 -async 1 -r 25 -v quiet %s" % (
        opt.videofile, num_sec_clip, os.path.join(opt.avi_dir, opt.reference, 'video.avi')))
    output = subprocess.call(command, shell=True, stdout=None)
    ...
    command = ("ffmpeg -y -i %s -ss 00:00:00 -to 00:00:0%s -qscale:v 2 -threads 1 -v quiet -f image2 %s" % (
        os.path.join(opt.avi_dir, opt.reference, 'video.avi'), num_sec_clip,
        os.path.join(opt.frames_dir, opt.reference, '%06d.jpg')))
    output = subprocess.call(command, shell=True, stdout=None)

    out_audio = os.path.join(opt.avi_dir, opt.reference, 'audio.wav')
    ...
    command = ("ffmpeg -y -i %s -ss 00:00:00 -to 00:00:0%s -ac 1 -vn -acodec pcm_s16le -ar 16000 -v quiet %s" % (
        os.path.join(opt.avi_dir, opt.reference, 'video.avi'), num_sec_clip,
        os.path.join(opt.avi_dir, opt.reference, 'audio.wav')))
    output = subprocess.call(command, shell=True, stdout=None)
    if output != 0:
        # 视频里没有音频轨道，去同目录下的.wav找音频
        command = ("ffmpeg -y -i %s -ss 00:00:00 -to 00:00:0%s -ac 1 -vn -acodec pcm_s16le -ar 16000 -v quiet %s" % (
            opt.videofile[:-4] + '.wav', num_sec_clip, os.path.join(opt.avi_dir, opt.reference, 'audio.wav')))
        output = subprocess.call(command, shell=True, stdout=None)
